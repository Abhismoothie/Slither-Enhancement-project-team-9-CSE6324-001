...
class SlitherReadStorage:
    ...
    #             size = var_type.size
    #             print(f"var_type: {var_type}")  # Log var_type
    #             print(f"var: {var}")  # Log var
    #             print(f"size: {size}")  # Log size
    #             print(f"slot_as_bytes: {slot_as_bytes}")  # Log slot_as_bytes
    #             if offset >= 256:
    #                 slot += 1
    #                 offset = 0
...
def _find_struct_var_slot(
    ...
    def _find_mapping_slot(
        ...
        :returns:
            ...
            slot (bytes): The storage location of the target variable.
            size (int): The size (in bits) of the target variable.
            offset (int): The size of other variables that share the same slowith type mappingt.

        """
        info = ""
        offset = 0
        print(f"Hereeeeeeeeeeeeee mapping slot: {target_variable_type}")  # Log target_variable_type
        if key:
            info += f"\nKey: {key}"
        if deep_key:
            info += f"\nDeep Key: {deep_key}"
        ...
        # mapping(elem => elem)
            ...
            raise NotImplementedError(
                f"{target_variable_type} => {target_variable_type.type_to} not hereeeeeeeeeeeee implemented"
            )
